...
#include <stdlib.h>
#include <string>
#include "platform-support/bundle_env.h"
#include "pbd/i18n.h"
...
fixup_bundle_environment (int, char* [], string & localedir)
{
	...
	Glib::setenv ("PATH", (std::string(ardour_dll_directory()) + G_SEARCHPATH_SEPARATOR + std::string(getenv ("PATH"))).c_str(), true);
...
static LONG WINAPI
unload_font_at_exception (PEXCEPTION_POINTERS)
{
	unload_custom_fonts ();
