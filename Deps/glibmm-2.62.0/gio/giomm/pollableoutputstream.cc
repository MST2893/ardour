...
PollableOutputStream& PollableOutputStream::operator=(PollableOutputStream&& src) noexcept
{
  Glib::Interface::operator=(std::move(src));
  return *this;
}

PollableOutputStream::~PollableOutputStream() noexcept
{}
...
{
  GError* gerror = nullptr;
  gssize retvalue = g_pollable_output_stream_write_nonblocking(gobj(), buffer, count, nullptr, &(gerror));
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
  return retvalue;
}
